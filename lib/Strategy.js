		...
		if ( this.isCallbackUrl( req.url ) ) {
			this._client.authorizationCallback( this.config.callbackUrl, req.query )
				...
				.then( ( userInfo ) => {
					...
					if ( arity === 9 ) {
						this._verify( req, this.config.remoteHost, userInfo.id, userInfo, null, this._tokenSet.accessToken,
							this._tokenSet.refreshToken, null, this.verifiedUser.bind( this ) );
					} else if ( arity === 8 ) {
						this._verify( req, this.config.remoteHost, userInfo.id, userInfo, this._tokenSet.accessToken,
							this._tokenSet.refreshToken, null, this.verifiedUser.bind( this ) );
					} else if ( arity === 7 ) {
						this._verify( req, this.config.remoteHost, userInfo.id, userInfo, this._tokenSet.accessToken,
							this._tokenSet.refreshToken, this.verifiedUser.bind( this ) );
					} else if ( arity === 5 ) {
						this._verify( req, this.config.remoteHost, userInfo.id, userInfo, this.verifiedUser.bind( this ) );
					} else { // arity === 4
						self._verify( req, this.config.remoteHost, userInfo.id, this.verifiedUser.bind( this ) );
					}
				} )
				.catch( ( err ) => {
					// eslint-disable-next-line no-console
