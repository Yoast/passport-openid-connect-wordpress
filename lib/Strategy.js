...
class Strategy {
	constructor( config, verify ) {
		this.name = "passport-wp-oauth-openid-connect";
	...
	init() {
		...
		openid.Issuer.defaultHttpOptions = { followRedirect: true };

		return openid.Issuer.discover( this.config.remoteHost )
			.then( ( issuer ) => {
				// TODO: Only pass relevant config to the client.
	 ...
	 */
	authenticate( req ) {
		if ( Strategy.isCallbackUrl( req.url ) ) {
			this._client.authorizationCallback( "http://localhost:3000/auth/yoast/callback", req.query )
				.then( ( tokenSet ) => {
	 ...
	 * @returns {void}
	 */
	static isCallbackUrl( url ) {
		let parsedUrl = urlUtil.parse( url );

		return parsedUrl.pathname === "/auth/yoast/callback";
	}
